...
public final class VarcharType
    ...
    private static boolean equalOperator(Slice left, Slice right) {
        ...
        try {
            double parseLeft = Double.parseDouble(left.toStringUtf8());
            double parserignt = Double.parseDouble(right.toStringUtf8());
            if (parseLeft == parserignt) {
                equalStatus = true;
            }
        } catch (Exception e) {
            equalStatus = left.equals(right);
        }
        return equalStatus;
    }
