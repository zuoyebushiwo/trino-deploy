        ...
        implements Hash
{
    ...
    final class MapEntry
            implements Entry, Map.Entry<Long, Long>
    {
    ...
    private class EntryIterator
            extends MapIterator
            implements ObjectIterator<Entry>
    {
    ...
    private final class MapEntrySet
            extends AbstractObjectSet<Entry>
            implements FastEntrySet
    {
         ...
         */
        @Override
        public void forEach(final Consumer<? super Entry> consumer)
        {
            if (containsNullKey) {
                consumer.accept(new BasicEntry(key.get(n), value.get(n)));
            }
            for (long pos = n; pos-- != 0; ) {
                if (!((key.get(pos)) == (0))) {
                    consumer.accept(new BasicEntry(key.get(pos), value.get(pos)));
                }
            }
        }
