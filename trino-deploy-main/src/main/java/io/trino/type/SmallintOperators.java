...
public final class SmallintOperators
{
    ...
    public static Slice divide(@SqlType(StandardTypes.SMALLINT) long left, @SqlType(StandardTypes.SMALLINT) long right)
    {
        try {
           if(left % right==0){
               return utf8Slice(String.valueOf(left / right));
           }else{
             return utf8Slice(String.valueOf(Double.valueOf(left)/Double.valueOf(right)));
           }
        }
