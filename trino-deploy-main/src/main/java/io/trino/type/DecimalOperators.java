...
public final class DecimalOperators {
    ...
    private static SqlScalarFunction decimalDivideOperator() {
        ...
        Signature signature = Signature.builder()
                .operatorType(DIVIDE)
                .longVariableConstraints(
                        longVariableExpression("r_precision", "min(38, a_precision + max(b_scale,1) + max(max(b_scale,1) - max(a_scale,1), 0))"),
                        longVariableExpression("r_scale", "max(max(max(a_scale, 1), max(b_scale, 1)),2)"))
                .argumentTypes(decimalLeftSignature, decimalRightSignature)
                .returnType(decimalResultSignature)
                .build();
        return new PolymorphicScalarFunctionBuilder(DecimalOperators.class)
    ...
    public static final class Add {
        ...
        public static Slice add(@SqlType(StandardTypes.VARCHAR) Slice left, @SqlType(StandardTypes.VARCHAR) Slice right) {
            BigDecimal multiply = new BigDecimal(left.toStringUtf8()).add(new BigDecimal(right.toStringUtf8())).stripTrailingZeros();
            return Slices.utf8Slice(multiply.toPlainString());
        }
    }
    ...
    public static final class Subtract {
        @SqlType(StandardTypes.VARCHAR)
        public static Slice add(@SqlType(StandardTypes.VARCHAR) Slice left, @SqlType(StandardTypes.VARCHAR) Slice right) {
            BigDecimal multiply = new BigDecimal(left.toStringUtf8()).subtract(new BigDecimal(right.toStringUtf8())).stripTrailingZeros();
            return Slices.utf8Slice(multiply.toPlainString());
        }
    }
    ...
    public static final class Multiply {
        @SqlType(StandardTypes.VARCHAR)
        public static Slice multiply(@SqlType(StandardTypes.VARCHAR) Slice left, @SqlType(StandardTypes.VARCHAR) Slice right) {
            BigDecimal multiply = new BigDecimal(left.toStringUtf8()).multiply(new BigDecimal(right.toStringUtf8())).stripTrailingZeros();
            return Slices.utf8Slice(multiply.toPlainString());
        }
    }
    ...
    public static final class Divide {
        @SqlType(StandardTypes.VARCHAR)
        public static Slice divide(@SqlType(StandardTypes.VARCHAR) Slice left, @SqlType(StandardTypes.VARCHAR) Slice right) {
            BigDecimal divide = new BigDecimal(left.toStringUtf8()).divide(new BigDecimal(right.toStringUtf8()), 10, HALF_UP).stripTrailingZeros();
            return Slices.utf8Slice(divide.toPlainString());
        }
    }
