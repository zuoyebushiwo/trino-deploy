...
import static io.trino.metadata.SignatureBinder.RelationshipType.*;
...
public class SignatureBinder
{
    ...
    SignatureBinder(Session session, Metadata metadata, TypeManager typeManager, Signature declaredSignature, boolean allowCoercion)
    {
        ...
        /**根据运算符情况进行隐式转换判断**/
        this.operator = declaredSignature.getName().toLowerCase(Locale.ENGLISH).replace("$operator$", "");
        typeCoercion.isComparableOperator = typeCoercion.canCoerceToVarchar(operator);
            ...
            for (int i = 0; i < parameters.size(); i++) {
                ...
                if (typeSignatureParameter.getKind() == ParameterKind.VARIABLE) {
                    if (bindings.containsLongVariable(typeSignatureParameter.getVariable())) {
                        ...
                        /**根据运算符情况进行隐式转换判断**/
                        operator = declaredSignature.getName().toLowerCase(Locale.ENGLISH).replace("$operator$", "");
                        typeCoercion.isComparableOperator = typeCoercion.canCoerceToVarchar(operator);
            ...
            implements TypeConstraintSolver
    {
        ...
        {
            /**根据运算符情况进行隐式转换判断**/
            declaredSignature.getName().toLowerCase(Locale.ENGLISH).replace("$operator$", "");
            typeCoercion.isComparableOperator = typeCoercion.canCoerceToVarchar(operator);
